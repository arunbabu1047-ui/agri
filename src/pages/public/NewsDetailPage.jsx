...
export default function NewsDetailPage() {
  ...
  return <Container maxWidth="md" className="detail-shell"><Link className="back-link" to="/news"><ArrowBackRoundedIcon fontSize="small" /> {t('common.back')}</Link><Stack direction="row" gap={1} sx={{ mt: 3, mb: 2 }}><Chip label={category?.[`name_${language}`]} className="category-chip" /><Typography variant="body2" color="text.secondary" sx={{ alignSelf: 'center' }}>{formatDate(item.date || item.created_at, language)} · {item.read_time}</Typography></Stack><Typography variant="h1" className="detail-title">{localized(item, 'title', language)}</Typography><Typography className="detail-summary">{localized(item, 'summary', language)}</Typography><Stack direction="row" justifyContent="space-between" alignItems="center" sx={{ my: 3 }}><Typography variant="body2" color="text.secondary">{t('common.by')} <b>{item.author_name || 'Agri Pulse editorial team'}</b></Typography><ButtonCopy onClick={copyLink} copied={copied} /></Stack><Box component="img" src={item.image} alt="" className="detail-image" /><Box className="article-body">{localized(item, 'body', language).split('\n\n').map((paragraph) => <Typography key={paragraph} paragraph>{paragraph}</Typography>)}</Box>{item.sample && <Alert severity="info" variant="outlined">{t('common.sample')} · This demonstration story is not current news or a verified advisory.</Alert>}<Divider sx={{ my: 4 }} /><Stack direction="row" gap={1} flexWrap="wrap">{item.tags?.map((tag) => <Chip key={tag} label={`#${tag}`} variant="outlined" size="small" />)}</Stack></Container>;
}

function ButtonCopy({ onClick, copied }) { return <button className="icon-text-button" onClick={onClick}><ContentCopyRoundedIcon fontSize="small" />{copied ? 'Copied' : 'Copy link'}</button>; }
