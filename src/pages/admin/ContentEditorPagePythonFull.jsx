...
import { Alert, Box, Button, Card, CardContent, FormControl, Grid, InputLabel, LinearProgress, MenuItem, Select, Stack, TextField, Typography } from '@mui/material';
...
export default function ContentEditorPage({ type }) {
  ...
  const submit = async (event, nextStatus = 'draft') => {
    ...
    try {
      ...
      setTimeout(() => navigate(`/admin/${type}`), 700);
    } catch (err) { setError(err.message || 'Unable to save this item.'); setSaving(false); }
  };
