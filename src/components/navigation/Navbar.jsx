...
export default function Navbar() {
  ...
  return <AppBar position="sticky" color="transparent" elevation={0} className="site-nav"><Container maxWidth="lg"><Toolbar disableGutters sx={{ gap: 2 }}>
    <Link to="/" className="brand"><span className="brand-mark"><GrassRoundedIcon fontSize="small" /></span><span><b>Agri</b> Pulse<small>KNOWLEDGE FOR THE FIELD</small></span></Link>
    ...
    <Stack direction="row" alignItems="center" gap={1} sx={{ ml: { md: 2 } }}><LanguageSwitcher /><Button component={Link} to="/admin/login" variant="contained" size="small" endIcon={<ArrowOutwardRoundedIcon />}>{t('nav.admin')}</Button><IconButton onClick={() => setOpen(true)} sx={{ display: { xs: 'inline-flex', md: 'none' }, color: 'primary.main' }} aria-label={t('nav.menu')}><MenuRoundedIcon /></IconButton></Stack>
  </Toolbar></Container>
  <Drawer anchor="right" open={open} onClose={() => setOpen(false)}><Box sx={{ width: 300, p: 2.5 }}><Stack direction="row" alignItems="center" justifyContent="space-between" sx={{ mb: 2 }}><Typography variant="h6" fontWeight={800}>Agri Pulse</Typography><IconButton onClick={() => setOpen(false)} aria-label="Close menu"><CloseRoundedIcon /></IconButton></Stack><List>{items.map(([path, label]) => <ListItemButton key={path} component={Link} to={path} onClick={() => setOpen(false)} sx={{ borderRadius: 2, mb: .5 }}><ListItemText primary={t(label)} /></ListItemButton>)}</List><Button component={Link} to="/admin/login" variant="contained" fullWidth sx={{ mt: 2 }} onClick={() => setOpen(false)}>{t('nav.admin')}</Button></Box></Drawer>
  </AppBar>;
}
